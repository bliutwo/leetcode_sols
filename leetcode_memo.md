...
- Reverse a linked list from position m to n. Do it in one-pass. Note: 1 ≤ m ≤ n ≤ length of list. Example: Input: 1->2->3->4->5->NULL, m = 2, n = 4; Output: 1->4->3->2->5->NULL
  - You need a "sentinel pointer", or dummy pointer, to point to the head of the list. You'll return `dummy.next` at the end, which is the head. You'll need to iterate through the list to create a pointer to the beginning of the sub-list that will be reversed. You'll also need a pointer to a node that will be reversed. Then you'll iterate through starting with first node to be reversed, and using three pointers, you'll change each arrow to point the opposite direction.
